...
use crate::{
    env::internal::{self, BytesObject}, unwrap::UnwrapInfallible, Bytes, BytesN,
    ConversionError, Env, IntoVal, TryFromVal, Val, Vec
};
...
impl Bls12_381 {
    ...
    pub fn g1_mul(&self, p0: Bytes, scalar_le_bytes: Bytes) -> Bytes {
        let env = self.env();
        let bin = internal::Env::bls12_381_g1_mul(env, p0.into(), scalar_le_bytes.into()).unwrap_infallible();
        Bytes::try_from_val(env, &bin).unwrap_infallible()
    }    

    pub fn g1_msm(&self, vp: Vec<Bytes>, vs: Vec<Bytes>) -> Bytes {
        let env = self.env();
    ...
    pub fn g2_mul(&self, p0: Bytes, scalar_le_bytes: Bytes) -> Bytes {
        let env = self.env();
        let bin = internal::Env::bls12_381_g2_mul(env, p0.into(), scalar_le_bytes.into()).unwrap_infallible();
        Bytes::try_from_val(env, &bin).unwrap_infallible()
    }    

    pub fn g2_msm(&self, vp: Vec<Bytes>, vs: Vec<Bytes>) -> Bytes {
        let env = self.env();
